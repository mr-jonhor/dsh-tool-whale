/**
 * dsh-tool-whale — Host-side Cordis plugin
 *
 * Injects the DeepSeek whale UI into the DSH web app index.html via
 * `webServer.tapIndex()`. The injected code consists of pure CSS animations
 * and a self-contained vanilla JS IIFE that creates the floating whale
 * element in the bottom-right corner of the page.
 *
 * Installation: add this plugin to your profile's cordis.patch.yml insert list.
 */

export const name = 'tool-whale'
export const inject = ['webServer']

// The full HTML snippet to inject (CSS + JS + SVG markup).
// Kept as a template literal here to avoid external file dependencies.
const WHALE_INJECTION = `
<style>
  @keyframes whale-swim {
    0%, 100% { right: 50px; }
    50% { right: 300px; }
  }
  @keyframes whale-float {
    0%, 100% { transform: translateY(0px); }
    50% { transform: translateY(-15px); }
  }
  @keyframes whale-turn {
    0%, 49% { transform: scaleX(1); }
    50%, 100% { transform: scaleX(-1); }
  }
  .dsh-whale-container {
    position: fixed;
    bottom: 35px;
    right: 50px;
    z-index: 9999;
    animation: whale-swim 24s ease-in-out infinite;
    cursor: pointer;
    user-select: none;
    transition: opacity 2s;
  }
  .dsh-whale-float {
    display: inline-block;
    animation: whale-float 3s ease-in-out infinite;
  }
  .dsh-whale-turn {
    display: inline-block;
    animation: whale-turn 24s ease-in-out infinite;
  }
  .dsh-whale-container:hover .dsh-whale-tooltip {
    opacity: 1;
    visibility: visible;
  }
  .dsh-whale-container.dsh-whale-paused {
    animation-play-state: paused;
  }
  .dsh-whale-container.dsh-whale-paused .dsh-whale-float {
    animation-play-state: paused;
  }
  .dsh-whale-container.dsh-whale-paused .dsh-whale-turn {
    animation-play-state: paused;
  }
  .dsh-whale-tooltip {
    position: absolute;
    bottom: 66px;
    right: 40px;
    background: rgba(0,0,0,0.85);
    color: white;
    padding: 8px 12px;
    border-radius: 8px;
    font-size: 12px;
    white-space: nowrap;
    opacity: 0;
    visibility: hidden;
    transition: opacity 0.2s, visibility 0.2s;
    pointer-events: none;
  }
  .dsh-whale-container.dsh-whale-hidden {
    opacity: 0;
    pointer-events: none;
  }
</style>
<script>
  (function() {
    if (document.getElementById('dsh-whale-root')) return;
    const root = document.createElement('div');
    root.id = 'dsh-whale-root';
    root.innerHTML = \`
      <div class="dsh-whale-container">
        <div class="dsh-whale-float">
          <div class="dsh-whale-turn">
            <svg width="80" height="42" viewBox="3.771 6.973 23.993 17.652" xmlns="http://www.w3.org/2000/svg">
              <path d="m27.501 8.469c-.252-.123-.36.111-.508.23-.05.04-.093.09-.135.135-.368.395-.797.652-1.358.621-.821-.045-1.521.213-2.14.842-.132-.776-.57-1.238-1.235-1.535-.349-.155-.701-.309-.944-.645-.171-.238-.217-.504-.303-.765-.054-.159-.108-.32-.29-.348-.197-.031-.274.135-.352.273-.31.567-.43 1.192-.419 1.825.028 1.421.628 2.554 1.82 3.36.136.093.17.186.128.321-.081.278-.178.547-.264.824-.054.178-.135.217-.324.14a5.448 5.448 0 0 1 -1.719-1.169c-.848-.82-1.614-1.726-2.57-2.435-.225-.166-.449-.32-.681-.467-.976-.95.128-1.729.383-1.82.267-.096.093-.428-.77-.424s-1.653.293-2.659.677a2.782 2.782 0 0 1 -.46.135 9.554 9.554 0 0 0 -2.853-.1c-1.866.21-3.356 1.092-4.452 2.6-1.315 1.81-1.625 3.87-1.246 6.018.399 2.261 1.552 4.136 3.326 5.601 1.837 1.518 3.955 2.262 6.37 2.12 1.466-.085 3.1-.282 4.942-1.842.465.23.952.322 1.762.392.623.059 1.223-.031 1.687-.127.728-.154.677-.828.414-.953-2.132-.994-1.665-.59-2.09-.916 1.084-1.285 2.717-2.619 3.356-6.94.05-.343.007-.558 0-.837-.004-.168.034-.235.228-.254a4.084 4.084 0 0 0 1.529-.47c1.382-.757 1.938-1.997 2.07-3.485.02-.227-.004-.463-.243-.582zm-12.041 13.391c-2.067-1.627-3.07-2.162-3.483-2.138-.387.021-.318.465-.233.754.089.285.205.482.368.732.113.166.19.414-.112.598-.666.414-1.823-.139-1.878-.166-1.347-.793-2.473-1.842-3.267-3.276-.765-1.38-1.21-2.861-1.284-4.441-.02-.383.093-.518.472-.586a4.692 4.692 0 0 1 1.514-.04c2.109.31 3.905 1.255 5.41 2.749.86.853 1.51 1.871 2.18 2.865.711 1.057 1.478 2.063 2.454 2.887.343.289.619.51.881.672-.792.088-2.117.107-3.022-.61zm.99-6.38a.304.304 0 1 1 .609 0c0 .17-.136.304-.306.304a.3.3 0 0 1 -.303-.305zm3.077 1.581c-.197.08-.394.15-.584.159a1.246 1.246 0 0 1 -.79-.252c-.27-.227-.463-.354-.546-.752a1.752 1.752 0 0 1 .016-.582c.07-.324-.008-.531-.235-.72-.187-.155-.422-.196-.682-.196a.551.551 0 0 1 -.252-.078c-.108-.055-.197-.19-.112-.356.027-.053.159-.183.19-.207.352-.201.758-.135 1.134.016.349.142.611.404.99.773.388.448.457.573.678.906.174.264.333.534.441.842.066.192-.02.35-.248.448z" fill="#4d6bfe"/>
            </svg>
          </div>
        </div>
        <div class="dsh-whale-tooltip">Click to close the whale !</div>
      </div>
    \`;
    document.body.appendChild(root);

    const container = root.querySelector('.dsh-whale-container');
    let isPaused = false;
    container.addEventListener('mouseenter', function() {
      if (isPaused) return;
      isPaused = true;
      container.classList.add('dsh-whale-paused');
    });
    container.addEventListener('mouseleave', function() {
      if (!isPaused) return;
      isPaused = false;
      container.classList.remove('dsh-whale-paused');
    });
    container.addEventListener('click', function() {
      container.classList.add('dsh-whale-hidden');
    });
  })();
</script>
`

export function apply(ctx) {
  const webServer = ctx.get('webServer')
  if (!webServer) {
    console.warn('[whale] webServer service not available, whale UI disabled')
    return
  }

  // Inject whale CSS + JS into every index.html response
  ctx.effect(() => webServer.tapIndex((html) => {
    return html.replace('</body>', WHALE_INJECTION + '\n  </body>')
  }), 'whale-ui: inject into index.html')

  console.log('[whale] whale UI plugin loaded — injected into index.html')
}
